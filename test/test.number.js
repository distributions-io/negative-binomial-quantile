...
describe( 'number quantile', function tests() {
	...
	it( 'should evaluate the quantile function of the Negative Binomial distribution', function test() {
		var actual;
		for ( var i = 0; i < data.length; i++ ) {
			actual =  quantile( data[ i ], r, p );
	...
	it( 'should return `NaN` if provided `NaN` as input', function test() {
		assert.isTrue( isnan( quantile( NaN, r, p ) ) );
	});

	it( 'should return `NaN` if provided a number outside [0,1]', function test() {
		assert.isTrue( isnan( quantile( 1.1, r, p ) ) );
		assert.isTrue( isnan( quantile( -0.1, r, p ) ) );
	});
